...
class SquatCounter(BaseExercise):
    ...
    def _apply_feedback_cooldown(self, current_time: float) -> str:
        """
        Blokuje migotanie UI. Zmiana komunikatu tylko po upływie cooldownu,
        CHYBA ŻE jest to komunikat krytyczny (np. zaliczenie punktu).
        """
        time_passed = current_time - self._last_feedback_time
        priority_messages = [self.fm.get("squat", "good_rep")]
